            ...
            "stocks": [
                {
                    ...
                    "avg_sentiment": round(stat["avg_sentiment"], 2)
...
def add_ai_provider_endpoint():
    ...
    success = add_ai_provider(
        data["provider"], data["api_key"], data.get("model"), set_active=True
    )
    return jsonify({"success": success})
...
def ask_chat_endpoint():
    ...
    try:
        ...
        thinking_instruction = thinking_instructions.get(
            thinking_level, thinking_instructions["balanced"]
        )
...
Focus on being informative and actionable."""
        ...
        if not provider:
            return jsonify(
                {"success": False, "error": "Failed to initialize AI provider"}
            ), 500
        ...
        try:
            ...
            logger.info(
                f"Chat response generated for {ticker}: {len(ai_answer)} characters"
            )
            ...
            return jsonify(
                {"success": False, "error": f"AI Provider Error: {str(e)}"}
            ), 500
...
def signup():
    ...
    from auth_manager import create_session, create_user
    ...
    if result["success"]:
        ...
        if session_result["success"]:
            response = make_response(
                jsonify({"success": True, "username": result["username"]})
            )
            response.set_cookie(
                "session_token",
...
def logout():
    ...
    if session_token:
        from auth_manager import delete_session
...
if __name__ == "__main__":
    # Initialize settings and auth tables on startup
    from auth_manager import init_auth_tables
